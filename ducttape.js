        ...
        if(files){
            ...
            files.forEach(function(file){
                if(__.loaded.indexOf(file)===-1){
                    __.getScript(window.location.origin+file+".js", function(){
                        __.loaded.push(file);
                        __.loading--;
                        if(__.loading<=0){
                            cb();
    ...
    ******************************************** */
    callAPI:function(url, p){
        $.support.cors = true;
            $.ajax({
                "crossDomain": true,
                "url": ((url.indexOf("http")===-1) ? this.config.base_api_url : "")+url,
                "method":((p.method) ? p.method : "GET"),
                ...
                "error":function(x,s,e){                    
                    p.response(x.responseJSON,null);
                }
            });
    },
    ...
    ******************************************** */
    loadComponent:function(componentId, params, cb){
        ...
        if(typeof __.components[componentId]==="undefined"){
            __.components[componentId]={"html":null, "js":null, "data":params};
            this.getContent(window.location.origin+"/components/"+componentId.toLowerCase()+"/ui.html", function(html){
                ...
                __.components[componentId].html=html;
                this.getScript(window.location.origin+"/components/"+componentId.toLowerCase()+"/logic.js", function(){ cb(); });
            });
        }else{
            __.components[componentId].data=params;
            cb();
        }
    ...
    ******************************************** */
    validateFormData:function(id){
        ...
        els.forEach(function(el){
            ...
            console.log("THIS",el.getAttribute("min") );
        ...
        loading:{
            ...
            section:function(id, hide){            
                if(hide){
                    ...
                    document.getElementById(id).innerHTML = document.getElementById(id).innerHTML+'<div id="'+id+'Spinner" style="width:100%; margin:50px 0 50px 0; text-align:center;"><span class="spinner-border mr-2"></span></div>';
                }
            }
        }
    }
